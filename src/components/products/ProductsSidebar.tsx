// src/components/products/ProductsSidebar.tsx
'use client'
import { usePathname } from 'next/navigation'
import Link from 'next/link'
import { Package, FileText, BookOpen } from 'lucide-react'

const SUB_ROUTES = new Set(['reports', 'templates'])

const NAV_ITEMS = [
  { label: 'Dashboard', icon: Package, path: '/products', exact: true },
  { label: 'Reports', icon: FileText, path: '/products/reports' },
  { label: 'Templates', icon: BookOpen, path: '/products/templates' },
]
...
export default function ProductsSidebar({ counts }: Props) {
  const pathname = usePathname()

  const isActive = (item: typeof NAV_ITEMS[0]) => {
    if (item.exact) {
      if (pathname === item.path) return true
      const seg = pathname.match(/^\/products\/([^/]+)/)?.[1]
      ...
      <aside
        ...
        <div className="px-3 py-3 border-b border-surface-200">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-semibold text-xs text-ink leading-tight truncate">Products</span>
          </div>
        </div>
          ...
          <div className="px-3 py-3 border-t border-surface-200 space-y-1">
            {counts.products != null && (
              <div className="flex justify-between text-[10px] text-ink-faint">
                <span>Products</span><span className="font-semibold text-ink">{counts.products}</span>
              </div>
            )}
            {counts.reports != null && (
              <div className="flex justify-between text-[10px] text-ink-faint">
                <span>Reports</span><span className="font-semibold text-ink">{counts.reports}</span>
              </div>
            )}
            {counts.templates != null && (
              <div className="flex justify-between text-[10px] text-ink-faint">
                <span>Templates</span><span className="font-semibold text-ink">{counts.templates}</span>
              </div>
            )}
          </div>
        )}
