...
export function Modal({ open, onClose, title, children, maxWidth = 'max-w-lg', zIndex }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={zIndex ? { zIndex } : undefined}>
      ...
      <div className={`relative w-full ${maxWidth} bg-surface-50 rounded-2xl shadow-float animate-scale-in max-h-[calc(100vh-2rem)] overflow-y-auto`}>
        {title && (
          <div className="flex items-center justify-between px-6 py-5 border-b border-surface-200/60">
